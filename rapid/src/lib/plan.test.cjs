...
// ────────────────────────────────────────────────────────────────
describe('writeGates', () => {
  it('creates GATES.json with per-wave planning gates from DAG', () => {
    const dag = {
      nodes: [
        { id: 'auth', wave: 1, status: 'pending' },
        { id: 'data', wave: 1, status: 'pending' },
        { id: 'api', wave: 2, status: 'pending' },
      ],
    ...
    writeGates(tmpDir, dag);
    const gatesPath = path.join(tmpDir, '.planning', 'sets', 'GATES.json');
    ...
    assert.ok(gates.gates['wave-1'], 'should have wave-1 gate');
    assert.ok(gates.gates['wave-2'], 'should have wave-2 gate');
  });

  it('initializes all gates with blocked status', () => {
    const dag = {
    ...
    writeGates(tmpDir, dag);
    const gates = JSON.parse(fs.readFileSync(path.join(tmpDir, '.planning', 'sets', 'GATES.json'), 'utf-8'));
    assert.equal(gates.gates['wave-1'].planning.status, 'blocked');
    assert.equal(gates.gates['wave-1'].execution.status, 'blocked');
  });

  it('sets required to the set IDs from each wave', () => {
    const dag = {
      nodes: [
        { id: 'x', wave: 1, status: 'pending' },
        { id: 'y', wave: 1, status: 'pending' },
      ],
      edges: [],
      waves: { '1': { sets: ['x', 'y'] } },
      metadata: { totalSets: 2, totalWaves: 1 },
    };
    writeGates(tmpDir, dag);
    const gates = JSON.parse(fs.readFileSync(path.join(tmpDir, '.planning', 'sets', 'GATES.json'), 'utf-8'));
    assert.deepStrictEqual(gates.gates['wave-1'].planning.required, ['x', 'y']);
    assert.deepStrictEqual(gates.gates['wave-1'].planning.completed, []);
  });
});
...
describe('checkPlanningGate', () => {
  it('returns open: false when wave is blocked', () => {
    const dag = {
      nodes: [{ id: 'a', wave: 1, status: 'pending' }],
      edges: [],
      waves: { '1': { sets: ['a'] } },
    ...
    writeGates(tmpDir, dag);
    const result = checkPlanningGate(tmpDir, 1);
  ...
  it('returns open: true when wave is open', () => {
    // Manually write an open gate
    const gatesData = {
      version: 1,
      gates: {
        'wave-1': {
  ...
  it('returns correct missing sets', () => {
    const gatesData = {
      version: 1,
      gates: {
        'wave-1': {
          planning: { required: ['a', 'b', 'c'], completed: ['a'], status: 'blocked' },
...
// ────────────────────────────────────────────────────────────────
describe('updateGate', () => {
  it('adds set to completed array', () => {
    const gatesData = {
      version: 1,
      gates: {
        'wave-1': {
          planning: { required: ['a', 'b'], completed: [], status: 'blocked' },
          execution: { status: 'blocked' },
        },
      },
    };
    ...
    assert.ok(updated.gates['wave-1'].planning.completed.includes('a'));
    assert.equal(updated.gates['wave-1'].planning.status, 'blocked', 'should remain blocked with missing set');
  });

  it('transitions wave to open when all sets are completed', () => {
    const gatesData = {
      version: 1,
      gates: {
        'wave-1': {
          planning: { required: ['a', 'b'], completed: ['a'], status: 'blocked' },
          execution: { status: 'blocked' },
        },
      },
    };
    ...
    assert.equal(updated.gates['wave-1'].execution.status, 'ready');
  });

  it('does not duplicate set in completed array', () => {
    const gatesData = {
      version: 1,
      gates: {
        'wave-1': {
    ...
    updateGate(tmpDir, 'a');
    const updated = JSON.parse(fs.readFileSync(path.join(tmpDir, '.planning', 'sets', 'GATES.json'), 'utf-8'));
    assert.equal(updated.gates['wave-1'].planning.completed.filter(s => s === 'a').length, 1);
  });
});
...
describe('decomposeIntoSets', () => {
  ...
  it('writes GATES.json to .planning/sets/', () => {
    const defs = makeMinimalSetDefs();
    decomposeIntoSets(tmpDir, defs);
    assert.ok(fs.existsSync(path.join(tmpDir, '.planning', 'sets', 'GATES.json')));
  });

  it('returns summary with sets, dag, ownership, manifest, gates', () => {
    const defs = makeMinimalSetDefs();
    ...
    assert.ok(result.gates, 'should have gates');
  });
