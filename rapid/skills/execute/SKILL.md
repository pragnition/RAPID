...
```bash
cat "$(node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" plan list-sets 2>/dev/null | node -e "const j=require('fs').readFileSync('/dev/stdin','utf-8');const d=JSON.parse(j);console.log(d.projectRoot || '.')")/.planning/sets/DAG.json" 2>/dev/null || echo '{"waves":{}}'
```
...
```bash
node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute resume {setName} 2>/dev/null | node -e "const d=JSON.parse(require('fs').readFileSync('/dev/stdin','utf-8'));console.log(JSON.stringify(d.handoff,null,2))"
```
...
- Run the resume command:
  ```bash
  node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute resume {setName}
  ```
...
```bash
node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" plan check-gate {waveNumber}
```
...
- If the gate is blocked and the user wants to override:
  ...
  node -e "const p=require(process.env.RAPID_TOOLS ? require('path').resolve(process.env.RAPID_TOOLS, '..', '..', 'lib', 'plan.cjs') : (process.env.HOME + '/RAPID/rapid/src/lib/plan.cjs'));p.logGateOverride(process.cwd(), {waveNumber}, {missingSetsList})"
  ```
...
```bash
node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" worktree create {setName}
```
...
```bash
node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute generate-stubs {setName}
```
...
2. Update registry phase:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute update-phase {setName} Discussing
   ```
...
1. Update registry phase:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute update-phase {setName} Planning
   ```
...
2. For each set in the wave, prepare the teammate:
   - Generate scoped CLAUDE.md:
     ```bash
     node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" worktree generate-claude-md {setName}
     ```
   - Update registry phase:
     ```bash
     node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute update-phase {setName} Executing
     ```
...
6. For each set, run verification (same as subagent mode):
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute verify {setName} --branch main
   ```
...
4. After the subagent returns, check the return status:
   ...
   **If return status is COMPLETE:**
   ...
   - Update registry phase based on results:
     ```bash
     # On success:
     node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute update-phase {setName} Done

     # On failure:
     node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute update-phase {setName} Error
     ```
...
5. Clean up stubs (for COMPLETE and BLOCKED, not for CHECKPOINT/paused):
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute cleanup-stubs {setName}
   ```
...
1. Run reconciliation (passing execution mode for wave summary metadata):
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/rapid/src/bin/rapid-tools.cjs}" execute reconcile {waveNumber} --mode "{executionMode}"
   ```
