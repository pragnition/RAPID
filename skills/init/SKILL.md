...
This step creates a foundation set (set 0) containing shared interfaces and stubs for multi-group parallel development. It is only applicable when team-size > 1.

**Gate: team-size check.**

If team-size is 1 (solo developer mode), skip this step entirely. Display: "Foundation set skipped (solo developer mode)."

**Gate: scaffold report check.**

Read `.planning/scaffold-report.json`. If the file does not exist (no scaffold has been run), skip this step. Display: "Foundation set skipped (no scaffold report found). Run /rapid:scaffold first if you want stub-based cross-group testing."

If both gates pass, proceed:

1. **Collect contracts.** Build a JSON object from the roadmapper's `contracts` array output (already available from Step 9). The object maps set IDs to their contract content:
   ```json
   {
     "set-id-1": { /* contract1 */ },
     "set-id-2": { /* contract2 */ }
   ...
   ```
   Construct this by iterating the `contracts` array from the roadmapper output, using each entry's `setId` as key and `contract` as value.

2. **Call the CLI to create the foundation set:**
   ...
   Where `<CONTRACTS_JSON>` is the stringified JSON object from step 1 above.

3. **Register the foundation set in STATE.json.** Add the foundation set to the milestone's sets array at index 0 (before all other sets) with status "pending" and branch "set/foundation":
...
4. **Regenerate DAG.json** to include the foundation node:
...
5. **Display:** "Foundation set created with shared interfaces from {N} sets." where {N} is the number of sets in the contracts object.
