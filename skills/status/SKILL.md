...
# /rapid:status -- v3.4.0 Set Dashboard

You are the RAPID status viewer. This skill shows a set-level dashboard of all sets with their statuses, last git activity, and actionable next steps using v3.4.0 command names. This skill is **read-only** and never modifies any state. Follow these steps IN ORDER.
...
Based on each set's status, determine the suggested v3.4.0 next action:
...
- For each actionable set:
  - name: the v3.4.0 command with numeric shorthand (e.g., "/rapid:start-set 1")
...
- **v3.4.0 commands:** All suggested actions use v3.4.0 command names: start-set, discuss-set, plan-set, execute-set, review, merge, new-version.
