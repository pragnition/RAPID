...
The user invokes this skill with a set identifier: `/rapid:execute-set <set-id>` (e.g., `/rapid:execute-set auth-system` or `/rapid:execute-set 1`).
...
If the user invoked `/rapid:execute-set <set-id> --fix-issues`, enter issue fix mode instead of normal execution:
...
If the user invoked `/rapid:execute-set <set-id> --retry-wave <wave-id>`, enter targeted retry mode:
...
> No job plans found for set '{set-id}'. Run `/rapid:discuss-set` and `/rapid:plan-set` first to create job plans.

Then use AskUserQuestion:
- **question:** "No job plans found"
- **options:**
  - "Run discuss" -- description: "Exit to run /rapid:discuss-set and /rapid:plan-set first"
  - "Cancel" -- description: "Exit without action"

Do NOT auto-trigger `/rapid:discuss-set` or `/rapid:plan-set`. Exit after the user responds.
...
- **options:**
  ...
  - "Pause execution" -- description: "Stop here to investigate. Resume with /rapid:execute-set {set-id}."
...
- "Cancel execution": Commit state and exit:
  > Execution cancelled. State saved. Resume with `/rapid:execute-set {set-id}`.
  Exit.
...
- **Discuss and plan are NOT part of this skill.** If JOB-PLAN.md files are missing, prompt the user to run `/rapid:discuss-set` and `/rapid:plan-set` first. Do NOT auto-trigger those commands.
...
- **--retry-wave:** Targets a specific wave for retry (e.g., `/rapid:execute-set 1 --retry-wave wave-2`). Verifies predecessor waves are complete before proceeding. After retrying the target wave, continues with subsequent waves.
