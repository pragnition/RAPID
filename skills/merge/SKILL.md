...
```bash
node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge order
```

Check current merge and execution status:

```bash
node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge status
```
...
1. Update merge status:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge update-status {setName} reviewing
   ```

2. Run programmatic validation:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge review {setName}
   ```
...
2. Update merge status:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge update-status {setName} cleanup
   ```

3. Spawn cleanup subagent using the Agent tool. Get worktree path from registry:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" worktree list 2>/dev/null | node -e "const d=JSON.parse(require('fs').readFileSync('/dev/stdin','utf-8'));const w=d.worktrees||{};const e=(Array.isArray(w)?w:Object.values(w)).find(x=>x.setName==='{setName}');console.log(e?.path||'NOT_FOUND')"
   ```
...
1. Execute the merge:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge execute {setName}
   ```
...
1. Run integration tests on main:
   ```bash
   node "${RAPID_TOOLS:-$HOME/RAPID/src/bin/rapid-tools.cjs}" merge integration-test
   ```
