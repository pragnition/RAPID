...
describe('handleBuildAgents integration', () => {
  ...
  it('generated planner agent has correct frontmatter', () => {
    const agentFile = path.join(AGENTS_DIR, 'rapid-planner.md');
    assert.ok(fs.existsSync(agentFile), 'rapid-planner.md should exist');
    const content = fs.readFileSync(agentFile, 'utf-8');

    assert.ok(content.includes('name: rapid-planner'), 'Should include agent name');
    assert.ok(content.includes('model: inherit'), 'Should include model setting');
    assert.ok(content.includes('color: blue'), 'Should include color');
    assert.ok(content.includes('<identity>'), 'Should have <identity> tag');
    assert.ok(content.includes('<role>'), 'Should have <role> tag');
  });

  it('assemble-agent CLI command no longer exists', () => {
    assert.throws(() => {
      execSync(`node "${CLI_PATH}" assemble-agent planner`, {
        encoding: 'utf-8',
        timeout: 5000,
      });
    }, 'assemble-agent should be an unknown command');
  });
});
...
describe('handleReview CLI dual-mode', () => {
  ...
  beforeEach(() => {
    ...
    // Create .planning/sets/auth-core directory structure
  ...
  it('review log-issue with 1 arg (set-id only) writes to set-level REVIEW-ISSUES.json', () => {
    ...
    const issuesPath = path.join(tmpDir, '.planning', 'sets', 'auth-core', 'REVIEW-ISSUES.json');
