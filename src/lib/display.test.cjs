...
describe('display', () => {
  ...
  describe('STAGE_BG', () => {
    ...
    it('execution stages (execute) use green background ANSI code', () => {
      const display = require(displayPath);
      assert.equal(
        display.STAGE_BG['execute'],
        '\x1b[102m',
        `Execution stage "execute" should use bright green background (\\x1b[102m)`
      );
    });
  ...
  describe('renderBanner', () => {
    ...
    it('execution stages use green background ANSI code in banner', () => {
      const display = require(displayPath);
      const result = display.renderBanner('execute');
      assert.ok(
        result.includes('\x1b[102m'),
        'Execution stage "execute" banner should contain bright green background code (\\x1b[102m)'
      );
    });
