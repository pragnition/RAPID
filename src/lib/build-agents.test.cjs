...
// All 26 roles (27 minus orchestrator removed in Phase 42)
const ALL_26_ROLES = [
...
// 4 core agents that get stubs (not full generation) -- orchestrator removed in Phase 42
const CORE_AGENTS = ['planner', 'executor', 'merger', 'reviewer'];
...
describe('build-agents', () => {
  ...
  describe('agent file generation', () => {
    it('generates exactly 26 .md files (22 generated + 4 stubs)', () => {
    ...
    it('build summary shows "Built 22 agents (4 core skipped)"', () => {
      assert.ok(
        buildOutput.includes('Built 22 agents (4 core skipped)'),
        `Build output should contain "Built 22 agents (4 core skipped)", got: ${buildOutput.trim()}`
      );
    });
  });
  ...
  describe('frontmatter validation', () => {
    it('each file starts with GENERATED or STUB comment then YAML frontmatter', () => {
      ...
      for (const file of files) {
        ...
        const isCore = content.startsWith('<!-- CORE: Hand-written agent');
        assert.ok(
          isGenerated || isStub || isCore,
          `${file} should start with GENERATED, STUB, or CORE comment, got: ${content.substring(0, 80)}`
        );
  ...
  describe('SKIP_GENERATION: core agent stubs', () => {
    it('core agent files start with STUB or CORE comment, not GENERATED', () => {
      for (const role of CORE_AGENTS) {
        ...
        assert.ok(
          isStub || isCore,
          `rapid-${role}.md should start with STUB or CORE comment, got: ${content.substring(0, 80)}`
        );
        assert.ok(
    ...
    it('core agent files have <role> section with content', () => {
      for (const role of CORE_AGENTS) {
        const content = fs.readFileSync(path.join(agentsDir, `rapid-${role}.md`), 'utf-8');
        const roleMatch = content.match(/<role>([\s\S]*?)<\/role>/);
        assert.ok(roleMatch, `rapid-${role}.md should have <role> tags`);
        assert.ok(
          roleMatch[1].trim().length > 0,
          `rapid-${role}.md <role> section should not be empty`
        );
      }
    });

    // Transition: Plans 02-03 will replace TODO placeholders with hand-written role content.
    // This test warns if any core agent still has a TODO placeholder but does not fail.
    it('core agents should eventually have hand-written role content (warns if TODO remains)', (t) => {
      for (const role of CORE_AGENTS) {
        const content = fs.readFileSync(path.join(agentsDir, `rapid-${role}.md`), 'utf-8');
        if (content.includes('<!-- TODO: Phase 42')) {
          t.diagnostic(`WARNING: rapid-${role}.md still has Phase 42 TODO placeholder -- Plans 02-03 will resolve this`);
        }
      }
    });
